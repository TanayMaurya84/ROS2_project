...
BT::NodeStatus GoToPose::onStart()
{
  ...
  if (!action_client_ptr_->wait_for_action_server(5s)) {
    RCLCPP_ERROR(node_ptr_->get_logger(), "NavigateToPose action server not available after waiting");
  ...
  RCLCPP_INFO(node_ptr_->get_logger(), "Sending goal: x=%f, y=%f", goal_msg.pose.pose.position.x, goal_msg.pose.pose.position.y);
